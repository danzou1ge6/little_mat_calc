...
impl Iterator for Permutation {
    ...
    fn next(&mut self) -> Option<Self::Item> { unsafe {
        ...
        if !found_ptwo { return None };

        let ptwo = self.perm.get_unchecked(ptwo_idx);
        let mut swap_idx = self.perm.len() - 1;
        while self.perm.get_unchecked(swap_idx) <= ptwo { swap_idx -= 1; }
...
pub fn det<T>(mat: &dyn Mat<Item=T>) -> Result<T, MatError> where T: LinearElem {
