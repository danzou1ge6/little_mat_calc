...
pub trait Mat{
    ...
    fn zeros(rows: usize, cols: usize) -> DataMatrix<Self::Item> where Self: Sized {
        DataMatrix::zeros(rows, cols)
    }
    /// Initialize an identity
    /// 
    /// For same reason as `zeros`, only [`DataMatrix`] can be used
    fn identity(rows: usize) -> DataMatrix<Self::Item> where Self: Sized {
    ...
    fn rows(&self) -> usize { if self.is_transposed() { self.cols_raw() } else { self.rows_raw() }}
    /// Return the number of cols, taken transpose into consideration
    fn cols(&self) -> usize { if self.is_transposed() { self.rows_raw() } else { self.cols_raw() }}
    ...
    fn transposed(mut self) -> Self where Self: Sized { (&mut self).transpose(); self }
    ...
    unsafe fn get_unchecked(&self, i: usize, j: usize) -> &Self::Item {
        if self.is_transposed() { self.get_unchecked_raw(j, i) } else { self.get_unchecked_raw(i, j)}
    }
    unsafe fn get_mut_unchecked(&mut self, i: usize, j: usize) -> &mut Self::Item {
        if self.is_transposed() { self.get_mut_unchecked_raw(j, i) } 
        else { self.get_mut_unchecked_raw(i, j)}
    }
    /// Get reference of (i, j), checking if the index is in range
    fn get(&self, i: usize, j: usize) -> Result<&Self::Item, MatError> {
        if i >= self.rows() || j >= self.cols() {
            return Err(IndexError{dim: self.dimensions(), i, j, mutable: false});
        }
    ...
    fn get_mut(&mut self, i: usize, j: usize) -> Result<&mut Self::Item, MatError> {
        if i >= self.rows() || j >= self.cols() {
            return Err(IndexError{dim: self.dimensions(), i, j, mutable: true});
        }
    ...
    /// Scale the matrix
    fn scale(&mut self, s: &Self::Item) -> &mut Self where Self: Sized{
        ...
        for i in 0..self.rows() {
            for j in 0..rhs.cols() {
                for k in 0..self.cols() {
                    result.get_mut_unchecked(i, j).ref_add_assign(
                        &self.get_unchecked(i, k).ref_mul(rhs.get_unchecked(k, j))
                    );
                }
            }
        }
    ...
    unsafe fn add_assign_unchecked(&mut self, rhs: &dyn Mat<Item=Self::Item>) {
        for i in 0..self.rows() { for j in 0..self.cols() {
            self.get_mut_unchecked(i, j).ref_add_assign(
                rhs.get_unchecked(i % rhs.rows(), j % rhs.cols())
            );
        }}
    ...
    unsafe fn sub_assign_unchecked(&mut self, rhs: &dyn Mat<Item=Self::Item>) {
        for i in 0..self.rows() { for j in 0..self.cols() {
            self.get_mut_unchecked(i, j).ref_sub_assign(
                rhs.get_unchecked(i % rhs.rows(), j % rhs.cols())
            );
        }}
    ...
    /// Same as `col`
    fn row(&self, i: usize) -> Result<SliceMatrix<Self::Item>, MatError> where Self: Sized {
        if i >= self.rows() {
            return Err(IndexError { dim: self.dimensions() , i, j: 0, mutable: true });
        }
        return unsafe { Ok(self.row_unchecked(i) )};
    }
    /// Check if `j` is too big, then call `col_unchecked`
    fn col(&self, j: usize) -> Result<SliceMatrix<Self::Item>, MatError> where Self: Sized {
        if j >= self.cols() {
            return Err(IndexError { dim: self.dimensions(), j, i: 0, mutable: true });
        }
    ...
    fn swap(&mut self, rhs: &mut dyn Mat<Item=Self::Item>) -> Result<(), MatError>{
        if self.rows() != rhs.rows() || self.cols() != rhs.cols() {
            return Err(InconsistentDimension { need: self.dimensions(), got: rhs.dimensions() });
        }
    ...
    fn clone_data(&self) -> DataMatrix<Self::Item>{
        ...
        for i in 0..self.rows() { for j in 0..self.cols() {
            unsafe {
                *result.get_mut_unchecked(i, j) = self.get_unchecked(i, j).clone()
            }
        }}
        result
    }
...
impl<T> PartialEq<dyn Mat<Item=T>> for dyn Mat<Item=T> where T: LinearElem {
