...
impl Display for MatError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            IndexError {dim, i, j, mutable}
                => write!(f, "Can't {} ({},{}) of a {:?} mat (or row or col)", if *mutable{"alter"}else{"read"}, i, j, dim),
            InconsistentDimension {need, got}
                => write!(f, "Need dimension {:?}, got {:?}", need, got),
            NotInvertable {rank, rows}
                => write!(f, "Can't invert a ({},{}) matrix with rank {}", rows, rows, rank),
            BadInitVector {len, cols, rows}
