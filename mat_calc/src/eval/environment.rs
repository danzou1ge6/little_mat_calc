...
use crate::token_pair::{TokenPair, TokenPairItem};
...
impl Environment {
    ...
    fn define_func(&mut self, name: String, param_names: TokenPairItem, body: TokenPairItem) {
            ...
            Pir(name_pair) => {
                if let Tok(Word(name)) = name_pair.first {
                    ...
                    return Err(EvalError::syntax(
                        "Parameter name list can only have Token::Word".to_string(),
                    ));
                }
            }
            Tok(Word(name)) => {
    ...
    fn apply_func(
        &mut self,
        func: &Function,
        parameters: TokenPairItem,
    ) -> Result<ObjectPairItem, EvalError> {
        if self.frames.len() == self.config.max_recursion {
            return Err(EvalError::recursion(format!(
                "Max recursion depth reached calling {}",
                func.name
            )));
        }
        ...
        if let Err(e) = self.bind_var(func.args.clone(), parameters) {
            self.pop_frame();
            return Err(if self.config.trace_back {
                e.cat_msg(format!("calling `{}`", func.name))
            } else {
                e
            });
        }

        let result = self.eval(func.body.clone()).map_err(|e| {
            if self.config.trace_back {
                e.cat_msg(format!(
                    "calling `{}` with `{}`",
                    func.name,
                    self.frame().format_func_args()
                ))
            } else {
                e
            }
    ...
    fn eval_n_times(
        ...
        if n == 1 {
            ...
            match parameters {
                Tok(_) => Err(EvalError::syntax(format!(
                    "Not enough arguments for builtin function, {} needed",
                    n
                ))),
                Pir(pair) => {
                    ...
                    return Ok(ObjectPairItem::List(Box::new(ObjectPair { first, second })));
                }
            }
        }
    }
    ...
    fn apply_builtin_func(
        ...
        self.push_frame();
        let args = self.eval_n_times(parameters, func.argn).map_err(|e| {
            self.pop_frame();
            if self.config.trace_back {
                e.cat_msg(format!("calling builtin `{}`", func.name))
            } else {
                e
            }
        })?;

        let result = (func.f)(args.clone(), self).map_err(|e| {
            self.pop_frame();
            if self.config.trace_back {
                e.cat_msg(format!("calling builtin `{}` with `{}`", func.name, args))
            } else {
                e
            }
        })?;
            ...
            Pir(pair) => {
                ...
                match sig {
                    Tok(token) => match token {
                        ...
                        _ => {
                            return Err(EvalError::syntax(
                                "Need a Token::Word as variable name".to_string(),
                            ))
                        }
                    },
                    Pir(pair) => match pair.first {
                        Tok(Word(name)) => {
                        ...
                        _ => {
                            return Err(EvalError::syntax(
                                "First item of function signature should be func name".to_string(),
                            ))
                        }
                    },
                }
    ...
    fn lambda(&mut self, args: TokenPairItem) -> Result<Function, EvalError> {
        ...
        match args {
            Tok(_) => {
                return Err(EvalError::syntax(
                    "Missing arguments for `lambda`".to_string(),
                ))
            }
            Pir(pair) => {
                return Ok(Function {
    ...
    fn _if(&mut self, args: TokenPairItem) -> Result<ObjectPairItem, EvalError> {
        ...
        match args {
            Pir(box TokenPair {
                ...
                if let Lit(Bool(b)) = test {
                    ...
                    return Err(EvalError::typ(format!(
                        "Test branch of `if` should return bool, not {}",
                        test
                    )));
                }
            }
            _ => {
                return Err(EvalError::syntax(
                    "Need 'test, then, else' branches for `if`".to_string(),
                ))
            }
        }
    }
    ...
    pub fn eval(&mut self, expr: TokenPairItem) -> Result<ObjectPairItem, EvalError> {
        ...
        match expr {
            Tok(token) => match token {
                ...
                Table(t) => {
                    ...
                    for name in t.data.into_iter() {
                        ot_data.push(self.find_object(&name).map_or(
                            Err(EvalError::name(format!("Name `{}` not found", name))),
                            |x| Ok(x),
                        )?);
                    }
                    return Ok(Lit(Literal::Table(Box::new(table::Table {
                        data: ot_data,
                        rows: t.rows,
                        cols: t.cols,
                    }))));
            ...
            Pir(pair) => {
                ...
                match first {
                    Func(func) => return self.apply_func(&func, pair.second.clone()),
